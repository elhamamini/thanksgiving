...
describe("/api/people routes", () => {
  ...
  describe("POST to /api/people", () => {
    it("should create a new person and return that persons information if all the required information is given", async () => {
      ...
      try {
        const frank = Person.create({ name: "frank", isAttending: true });
    ...
    it("should return status code 400 if missing required information", async () => {
      try {
        const frank = Person.create({ nam: "frank" });
        await request(app)
          .post("/api/people")
          .send(frank)
          .set("Accept", "application/json")
  ...
  describe("PUT to /api/people/:id", () => {
    it("should update a persons information", async () => {
      try {
        ...
        const frankUpdate = await request(app)
          .put("/api/people/1")
          .send({
            isAttending: false
          })
          .expect("Content-Type", "/json/")
          .expect(200);
        expect(frankUpdate.isAttending).toBe(false);
      } catch (err) {
        fail(err);
      }
    });
...
describe("DELETE to /api/people/:id", () => {
  it("should remove a person from the database", async () => {
    try {
      ...
      const removePerson = await request(app)
        .delete("/api/people/1")
        .expect("Content-Type", /json/)
        .expect(200);
      expect(removePerson.length).toBe(2);
    } catch (err) {
      fail(err);
    }
  });
